...
const SideBar = ({ isCollapsed, users }: SideBarProps) => {
  ...
  return (
    <section className="group flex relative flex-col h-full gap-4 p-2 data-[collapsed=true]:p-2 max-h-full overflow-auto bg-background">
      {!isCollapsed && (
      ...
      <div className="mt-auto">
        <div className="flex justify-between items-center gap-2 md:px-6 py-2">
          ...
          <div>
            <LogoutLink onClick={() => setIsLoading(true)}>
              {isLoading ? (
                <Loader2
                  size={22}
                  className="animate-spin select-none pointer-events-none"
                />
              ) : (
